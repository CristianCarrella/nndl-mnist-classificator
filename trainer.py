...
class Trainer:
    ...
    def train(self):
        global iterations, best_val_loss, best_iteration_case
        ...
        for e in range(self.hyper_params.epochs):
            ...
            if num_of_validation_try > params.patience or e == params.epochs - 1:
                log_results({"id": iterations, "type": "train", "number_of_epochs" : e, "val_loss": prev_val_loss, "val_acc": validation_accuracy})
                if prev_val_loss < best_val_loss:
                    best_val_loss = prev_val_loss
                    best_iteration_case = iterations
    ...
    def test(self):
        ...
        disp.plot()
        plt.savefig(f'./confusion_matrix/{iterations}_confusion_matrix.png', format='png', dpi=300)
        plt.close()

        accuracy = (good_test / total_test) * 100
        print(f"Test Accuracy: {good_test}/{total_test} ({accuracy:.2f}%)")
        log_results({"id": iterations, "type": "test", "test_accuracy" : accuracy})
    ...
    def plot_training_graph(self):
        ...
        best_epoch = self.validation_losses.index(min(self.validation_losses))
        best_loss = self.validation_losses[best_epoch]

        print("Making plot...")
        plt.scatter(best_epoch, best_loss, color='red', label=f"Early Stopping (Epoch {best_epoch + 1})", zorder=5)
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f"./functions_plots/{iterations}_function.png", format='png', dpi=300)
        plt.show()
        plt.close()
